...
function watcher() {
   ...
   gulp.watch(path.watch.images, { ignorePermissionErrors: true }, procImages).on('unlink', (currentPath) => {
   ...
   gulp.watch(path.watch.styles, { ignorePermissionErrors: true }, styles).on('unlink', (currentPath) => {
      path.clearForTask(currentPath, path.styles.dest);
   })
   gulp.watch(path.watch.js, { ignorePermissionErrors: true }, js).on('unlink', (currentPath) => {
...
export { zip };
export { zipPl };
export const zipWpPl = gulp.series(zipPl, (done) => { app.isWP = true; return done() }, zip);

export { ftp };
export { ftpPL };
export const ftpWpPl = gulp.series(ftpPL, (done) => { app.isWP = true; return done() }, ftp);
