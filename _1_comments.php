...
<div class="comments" id="comments">
		...
		<div class="comments_containlker cf">
			<div class="comments_name cf">
				<div class="comments_title">
					<?php comments_number( '0 ' . esc_html__( 'Comments', 'ale' ), '1 ' . esc_html__( 'Comment', 'ale' ), '% ' . esc_html__( 'Comments', 'ale' ) ) ?>
				</div>
	...
	<?php if ( comments_open() ) { ?>
		<div id="respond" class="leave-a-comment">
			...
			<?php else : ?>
				<form action="<?php echo esc_url( get_option( 'siteurl' ) ); ?>/wp-comments-post.php" id="comment-form" method="post"
					class="comment-form cf">
						...
						<div class="line-item cf">
							<div class="third_item">
								<input type="text" name="author" id="author" tabindex="2" <?php if ( $req )
									...
									required="required" placeholder="<?php esc_html_e( 'Your name*', 'ale' ); ?>" />
							</div>
							<div class="third_item">
								<input type="email" name="email" id="email" tabindex="3" <?php if ( $req )
									echo "aria-required='true'"; ?>
									required="required" email="true" placeholder="<?php esc_html_e( 'Your e-mail*', 'ale' ); ?>" />
							</div>
